...
class multi_fluka:
    """
        Class for summing together results from different Fluka simulations 
        with the exact same binning
    
    """
    
    def __init__(self, filenames, path_data): 
        
        self.path_data = path_data
        self.filenames = filenames
        self.fluka_contributions = []
        self.survey = [] 
        self.sampling_path = []
        self.result = [] 
        self.yesContact = []
    ...
    def set_survey(self,  filenameSurvey, path_data , origo):
        self.survey = Survey(filenameSurvey, self.path_data, origo )
    
    
    def set_sampling_path(self, sampling_path):
    ...
    def setup(self):
        ...
        for filename in self.filenames:
            fluka = USRBIN(filename, self.path_data, 0.0036)
            fluka.readError()
            self.fluka_contributions.append(fluka)

        assert len(self.fluka_contributions) > 1


        self.result = self.fluka_contributions[0] 
        for fluka in self.fluka_contributions[1:]:
            self.result.cube = self.result.cube + fluka.cube
        
        for x in range(self.result.cube.shape[0]):
           for y in range(self.result.cube.shape[1]):
               for z in range(self.result.cube.shape[2]):
                   
                   # final error = (fluka_1.error^2 + fluka_2.error^2 + ... + fluka_N.error^2) ^(1/2)
                   error = self.result.cubeErrors[x,y,z]**2
                   for fluka in self.fluka_contributions[1:]:
                       error = error + fluka.cubeErrors[x,y,z]**2
                       
                   error_final = math.sqrt(error)
                   
                   self.result.cubeErrors[x,y,z] = error_final
                
        print("Result contain summation of contributions from {}".format(self.filenames))


    def valuesofpath(self):
        
        self.yesContact = np.zeros(self.result.cube.shape[2])
        self.yesContactError = np.zeros(self.result.cube.shape[2])
        self.yes40cm = np.zeros(self.result.cube.shape[2])
        self.yes40cmError = np.zeros(self.result.cube.shape[2])  
        sampling_path = self.sampling_path
        
        
        
        zStart = int(self.result.info["zmin"][0])
        z_bin_width = round(self.result.info["zwidth"][0])
        
        idx = 0
        for z in range(self.result.cube.shape[2]):

            self.yesContact[z] = self.result.cube[sampling_path[idx][1],0,z]
            self.yesContactError[z] = self.result.cubeErrors[sampling_path[idx][1],0,z]
            self.yes40cm[z] = self.result.cube[sampling_path[idx][1] - 40 ,0,z]
            self.yes40cmError[z] = self.result.cubeErrors[sampling_path[idx][1] - 40 ,0,z]
        

            if z*z_bin_width + zStart > sampling_path[idx][2]:
                idx += 1
    ...
    def highlight_path(self):
        ...
        zStart = int(self.result.info["zmin"][0])
        idx = 0
        self.cubePath = self.result.cube.copy()
        for z in range(self.result.cube.shape[2]):
            self.cubePath[sampling_path[idx][1],0,z] = pathValue
            if z*10 + zStart > sampling_path[idx][2]:
                idx += 1

        
        image = self.cubePath[0:, 0, 0:] 
        
        fig = plt.figure()
        ax = plt.subplot(111)
        
        
        image[ image != 400000] = 0
        
        
        plt.pcolor(self.result.X, self.result.Y, image, cmap = "Purples")
        if self.result.X == []:
            print("Load Geo first")
        else:
            self.result.drawGeo(ax)
            
        
        plt.ylabel("x[cm]", fontsize = 16)
        plt.xlabel("z[cm]", fontsize = 16)
        
        z_Start = int(self.result.info["zmin"][0])
        z_Stop = int(self.result.info["zmax"][0])
        
        plt.xlim(z_Start ,z_Stop)
        plt.ylim(-60,0)
        xlength = 15
        
        
        plt.title("Sampling Path", fontsize = 20)
    ...
    def set_AD6_errors(self, AD6error):
    

        self.AD6_abs_error_contact = np.array(self.survey.yesContact)*AD6error

        function_of_relevant_errors = interp1d(self.result.xcoordinates/100, self.yesContactError/100 ) #second vector in fraction instead of percent 
        ...
        self.AD6_abs_error_40cm = np.array(self.survey.yes40cm)*AD6error
    ...
    def ratios_in_range(self, ax, xmin, xmax):
        ...
        ferrors = interp1d(self.result.xcoordinates/100, self.yesContactError/100 )
        ...
        ferrors = interp1d(self.result.xcoordinates/100, self.yes40cmError/100 )
    ...
    def mean_by_range(self, element_limit):
        ...
        xes = self.result.xcoordinates/100
    ...
    def plot_full(self):
        ...
        ax2 = ax.twinx()
        ax2 = self.result.drawGeo_inside_of(ax2 , 0.50, xmax , 100, 0.3, 0.3)
        ...
        ratiosContactresult = g(self.survey.xesContact)/self.survey.yesContact
        ratios1meterresult = f(self.survey.xes40cm)/self.survey.yes40cm
        ...
        weightedMean, sd = self.weightedMean(ratios1meterresult , rel_ratioerrors)
        #sub_ratiosContact, sub_weightedMean_contact, sub_sd_contact, sub_ratios40cm, sub_weightedMean_40cm, sub_sd_40cm = self.mean_by_range([xmin*100, xmax*100])
        
        plt.errorbar(self.survey.xesContact, ratiosContactresult, yerr = ratiosContactresult *rel_ratioerrors_contact , label = "Contact Fluka/Measurements" , marker=".", linestyle= 'None', markersize = 12)

        

        plt.errorbar(self.survey.xes40cm, ratios1meterresult , yerr = ratios1meterresult *rel_ratioerrors_40cm , label =  "40 cm Fluka/Measurements", marker=".", linestyle= 'None', markersize = 12)
    ...
    def plot_triplet(self):
        ...
        ax2 = ax.twinx()
        ax2 = self.result.drawGeo_inside_of(ax2 , 0.43, xmax , 100, 0.05, 0.4)
