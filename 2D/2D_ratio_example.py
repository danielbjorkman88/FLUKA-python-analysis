...
vmin = 0.1
vmax = 1/vmin #this centers the colormap to 1
...
fluka2 = USRBIN("ir1_1m", path_data, normfactor)
...
image = fluka.cube[0:,y_beamaxis, 0:] / fluka2.cube[0:,y_beamaxis, 0:]
plt.pcolor(fluka.X, fluka.Y,image, norm=LogNorm(vmin=vmin, vmax=vmax) ,cmap = ratio_cmap)
...
plt.title("Ratio 2D plot example" , fontsize = 25)
plt.gcf().subplots_adjust(bottom=0.1)

ax = plt.subplot(gs[18:, 0])

img = plt.pcolor(fluka.X, fluka.Y,image,  shading='auto' ,norm=LogNorm(vmin=vmin, vmax=vmax) ,cmap = ratio_cmap)
img.set_visible(False)
plt.colorbar(orientation="horizontal", cax=ax)

plt.gcf().subplots_adjust(bottom=0.3)
plt.xlabel("Ratio dose rate", fontsize = 20)
...
try:
    os.chdir(path)
    plt.savefig("2D_ratio_example_plot.pdf",  bbox_inches = 'tight', pad_inches = 0.1)
except:
    pass;
